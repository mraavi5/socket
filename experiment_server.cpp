...
int main() {
    ...
    std::string algorithm((std::istreambuf_iterator<char>(file_alg)), std::istreambuf_iterator<char>());
    ...
    for (;;) {
        ...
        if (redisReply.isOk() && redisReply.toString().length() > 0) {
            ...
            socket.send_to(boost::asio::buffer(reply), sender_endpoint);
            if (reply.length() < MaxFrameSize - ChecksumSize) {
                // If the reply is shorter than the maximum frame size, then we are done
                return 0;
            }
        } else {
            std::string reply = "N";
            socket.send_to(boost::asio::buffer(reply), sender_endpoint);
            return 0; // Terminate upon receiving an invalid request
        }
    }
